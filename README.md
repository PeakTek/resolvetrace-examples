...
| [`web/platform-consent-demo`](./web/platform-consent-demo) | Vite + TypeScript (vanilla, no framework) | A user-driven **record → review → submit** demo built on the SDK's report widget in `record` mode: buffered replay (nothing uploads until Submit), tier-adaptive (Platform = multi-clip curation + consent; OSS = whole-session single clip). Served by a managed backend. |
...
> starting point. `web/platform-consent-demo` shows the record-and-submit widget
